...
* Create an empty project with name `golang-sdk-tutorial` and run the following command in the command line:
  ...
  > **Note**: Upon completion, it will add a `go.mod` file in the project folder.
...
> **Note**: This tutorial uses Auth Page(IDX) for authentication, where Registration and Login functionality is already implemented.
...
> **Note**: return_url can be your website, frontend app, or backend server url where you are handling the access token.

## Retrieve User Data using Access Token

> **Note**: Once the authentication is done using Auth Page (IDX), the default script of LoginRadius sends an access token in the query string as a token parameter with the return_url. The return_url will access the Golang backend API with query parameter (access token).
>
...
> **Note**: In addition to Registration and Login actions, the Auth Page (IDX) supports more actions. Refer to <a href="https://www.loginradius.com/docs/developer/concepts/idx/" target="_blank">this document</a> for more information.
