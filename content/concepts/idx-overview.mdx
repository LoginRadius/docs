...
**Auth Page (IDX)** is a ready-to-use web page available on a dedicated instance in LoginRadius and allows you to setup a centralized authentication page. It is a set of registration, login, and related web pages such as forgot password, profile, etc.
...
- Centralized - You will be able to handle multiple sites' Login and Registration flow in a single location and service multiple properties with a single set of styles and configurations.
...
> **Note**: Replace `<LoginRadius APP Name>` with The LoginRadius APP Name that you can find on [Setup your Dashboard](/howto/dashboard-setup/) page and the `<Return URL>` with the location you would like to direct customers after completing the action.
