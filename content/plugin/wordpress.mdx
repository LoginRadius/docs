...
Default WP Login Form is used for admin purposes. If the admin wants to bypass the LoginRadius authentication services or doesn't require registering a customer at LoginRadius, it can be done using the **[ciam_wp_default_login]** shortcode. In order to accomplish this, Admin needs to create a separate page and apply the given shortcode on it.
...
You can use the theme’s CSS file to customize the design (layout, pop-up design, or interface element) of LoginRadius interfaces like Login, Register, Forgot Password, etc. 

### FAQ

#### Where to look up user details in the database?

- You can see the user's email id and username in the "wp_users" table.
- Check plugin options setting in the "wp_options" table. 
...
To update the plugin, first deactivate the previous plugin; then replace the codebase and activate the new plugin. Alternatively, you can also update it by clicking the **update** of LoginRadius CIAM at “/wp-admin/plugins.php” of your site.
