...
Single sign on (SSO) is the authentication mechanism which allows users to sign in different software systems.  After signing into one application user is signed into another application automatically. 
...
Let’s take an example, We login into one of our applications, on successful login we receive an access token and after that we can actually redirect to the profile page or post login page to the user.


## Configuration

To use LoginRadius Web SSO, make sure the respective domain is whitelisted under Domain Whitelisting under LoginRadius dashboard.
...
This section covers how you can manually set a LoginRadius access_token for SSO if the access_token was obtained by other means.
